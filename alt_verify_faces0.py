            ...
            if result["verified"]:
                ...
                for cluster in clusters:
                    if i in cluster:
                        ...
                        # Save images as soon as a match is found
                        shutil.copy(image_paths[j], os.path.join(directory_path, f"person{clusters.index(cluster) + 1}"))
                        break
                ...
                if not found_cluster:
                    ...
                    # Create a new directory for the new cluster
                    os.makedirs(os.path.join(directory_path, f"person{len(clusters)}"), exist_ok=True)

                    # Save images in the new cluster
                    shutil.copy(image_paths[i], os.path.join(directory_path, f"person{len(clusters)}"))
                    shutil.copy(image_paths[j], os.path.join(directory_path, f"person{len(clusters)}"))
